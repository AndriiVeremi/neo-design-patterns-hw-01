...
export class BorrowService {
  borrow(reader: Reader, copy: Copy): string {
    ...
    return `Copy of "${copy.getBook().getTitle()}" successfully borrowed by ${reader.getName()}`;
  }
  ...
  returnBook(reader: Reader, copy: Copy): string {
    ...
    return `Copy of "${copy.getBook().getTitle()}" successfully returned by ${reader.getName()}`;
  }
}
